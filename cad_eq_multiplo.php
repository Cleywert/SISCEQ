...
if (isset($_POST['cad'])) {
	...
	if ($flag) {
		...
		for ($j = 0; $j < $q; $j++) {
			...
			$query = "INSERT INTO equipamento(num_serie,tipo,descricao,marca,modelo,status,condicao_entrada) VALUES ('" . $_POST['serie' . $j] . "','" . $tipo . "','" . $_GET['descricao'] . "','" . $marca . "','" . $_GET['modelo'] . "'," . $status . "," . $_GET['condicao_entrada'] . ")";
			$result = mysqli_query($conn, $query);
			if (!$result)
				$flag = false;
		}
